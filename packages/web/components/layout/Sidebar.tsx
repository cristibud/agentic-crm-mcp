...
export function Sidebar({ collapsed, onToggle }: SidebarProps) {
  ...
  const handleLogout = () => {
    Cookies.remove('crm_api_key');
    router.push('/login');
  };

  return (
    <aside
      className={`flex flex-col bg-gray-900 text-white transition-all duration-200 ${collapsed ? 'w-16' : 'w-60'} min-h-screen`}
    >
      {/* Logo */}
      <div className={`flex items-center h-16 px-4 border-b border-gray-700 ${collapsed ? 'justify-center' : 'justify-between'}`}>
        {!collapsed && (
          <span className="text-lg font-bold text-indigo-400 truncate">CRM MCP</span>
        )}
        <button
          onClick={onToggle}
